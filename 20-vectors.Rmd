...
The first subsetting vector, x[-which(x > 0)], works by returning a vector of negative indices in the vector x that are greater than zero, which are subsequently dropped when used to subset x. On the other hand, x[x <= 0] returns a vector of Booleans where TRUE is all cases where x is less than or equal to zero. If x is a heterogenous list of character and numeric values, x[-which(x > 0)] does not return an indice referring to the character value (since it is not greater than zero) and it is therefore not dropped during subsetting. In contrast, x[x <= 0] returns "NA" for the character value, which results in a NULL output:
...
Draw the following list as a nested set: list(a, b, list(c, d), list(e, f)). 

image:

![Caption](docs/20-vectors/list_1.png)

Draw the following list as a nested set: list(list(list(list(list(list(a)))))). 

![Caption](docs/20-vectors/list_2.png)
...
``` {r 20.5.2a}
# As with lists, different subsetting operators extract different aspects of tibbles. Double brackets simplify the output to an atomic vector while single brackets preserve the tibble structure:
...
A tibble is a type of list, but unlike a list it is always two-dimensional and always composed of equal length vectors.
...
```{r 20.7.1}
# It returns a "difftime" atomic vector of 3600 which prints as 01:00:00

(x <- hms::hms(3600))

str(x)

# The augmented vector is built on top of a double vector: 

typeof(x)

# Its classes are "hms" and "difftime." It has a units attribute which indicates that the value is in seconds. 
...
# tibble() will recycle a vector of length 1 to create a tibble: 
...
``` {r 20.7.3}
# Yes, as long as the list is the same length as the other vectors: 

y <- tibble(x = 1:5, y = list(list(1:5), "string", 1:100, TRUE, 2))
```
